...
#   "X.*"  장 머리말 + 그 장의 모든 절
#   "X.Y"  그 절 (+ 장 머리말 자동 포함)
# 10.2(2차 상담 이관 기준)는 모든 카테고리에 넣는다. 이관은 범위 밖 문의에서만
# 일어나지 않는다 — 반복 문의·보상 요구·연동 오류 지속은 업무 카테고리 안에서
# 벌어지는 일이고, 기준이 근거에 없으면 계획 단계가 판단할 수가 없다.
ALWAYS: list[str] = ["이 매뉴얼을 쓰는 방법", "0.*", "10.2", "10.3", "11"]
...
ROUTE_SECTIONS: dict[str, list[str]] = {
    ...
    "SPEC_SHIPPING": ["3.*", "5.1", "5.2", "5.4", "6", "7"],
    "OTHER": ["10.1", "10.2"],
}
