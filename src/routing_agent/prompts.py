...
- **이전 대화가 있으면 그 사안의 카테고리를 잇는다.** "그대로예요", "다시 해봤어요",
  "확인했어요" 처럼 그 자체로는 뜻이 없는 후속 발화는 앞에서 다루던 사안의 카테고리다.
  고객이 화제를 분명히 바꿨을 때만 카테고리를 바꾼다 — 후속 발화를 새 문의로 취급하면
  같은 사안을 몇 번 물었는지 알 수 없고, 반복 문의 이관 기준(§10.2)이 성립하지 않는다.
...
- **대화가 길다는 것은 반복 문의가 아니다.** §10.2 의 반복 기준은 고객이 **같은 사안**을
  다시 묻는 경우에만 적용한다. 앞에서 다른 것을 물었다면 이번 문의는 처음 나온 사안이다.
...
- **자료를 가리키는 말은 답변에 쓰지 않는다.** "매뉴얼", "스냅샷", "목데이터",
  "조회 결과에 따르면" 처럼 상담원이 무엇을 보고 있는지 가리키는 표현이 그렇다
  (예: "매뉴얼 스냅샷 값" → "기본 운임"). 고객은 그 자료를 알지 못한다.
  다만 **서비스 용어는 그대로 쓴다** — 수집·집하·운송장출력·연동처럼 화면과 안내에
  나오는 말은 바꾸면 오히려 뜻이 흐려진다. 조건·절차·수치도 빼지 않는다.
