...
- **보낼 물건의 이름만 있고 서비스를 가리키는 말이 없는 문의는 SPEC_SHIPPING 이다**
  — 반입 제한을 묻는 것이다("딸기 보내려는데 되나요?"). 서비스를 가리키는 말이
  하나라도 있으면 그 서비스의 카테고리다.
...
- CLOSE    : 고객이 더 물을 것이 없다고 했다. 인사하고 대화를 맺는다.
...
- **CLOSE 는 끝맺음 신호가 분명할 때만 고른다.** "없어요", "괜찮아요", "됐어요",
  "고맙습니다" 처럼 **더 물을 것이 없다는 뜻이 드러난** 응답이 그렇다. 이때는 이관이
  아니다 — 대화가 끝난 것이지 우리가 못 푼 것이 아니다.
  **물음이 없다는 것만으로는 CLOSE 가 아니다.** "처음이라 모르겠어요" 처럼 도움을
  청하는 말도 물음이다. 먼저 답하고, 끝맺음 인사는 그 뒤에 붙인다.
- **뜻을 잡을 수 없는 발화는 ASK 다.** 내용이 없거나("네?", "ㅠㅠ"), 앞 턴을 가리키는데
  그 대상이 이력에 없으면("그걸로 할게요", "거기로 보내주세요", "아까 그 날짜로"),
  무엇을 말씀하시는지 되묻는다. 이런 발화를 CLOSE 로 끝내거나 OUT_OF_SCOPE 로
  돌리면, 고객은 아직 할 말이 남았는데 대화가 닫힌다. 모르면 되묻는다(§0 원칙3).
- **새 예약을 대신 넣지 않는다.** 예약은 고객이 예약 화면에서 직접 하는 일이다(§2:
  로그인 → 서비스 선택 → 택배사 선택 → 정보 입력 → 약관 동의 → 결제). 우리에게는
  예약을 접수하는 도구가 없다. "예약을 진행하겠습니다", "대신 예약해 드리겠습니다" 처럼
  **할 수 없는 일을 하겠다고 말하지 않는다.** 예약 방법을 안내하는 것까지가 우리 몫이다.
...
- **근거 문서에 없는 낱말·규정을 아는 척하지 않는다.** 고객이 쓴 말이 근거 어디에도
  없으면 "~는 ~후에 가능합니다" 처럼 규정을 만들어 답하지 말고, 무엇을 말씀하시는지
  되묻는다. 뜻이 여럿인 낱말은 어느 쪽인지 확인한다(§0 원칙3: 근거가 없는 사항은
  만들어내지 않는다).
- **우리가 할 수 없는 일을 하겠다고 말하지 않는다.** 새 예약 접수·결제가 그렇다.
  예약을 원하시면 예약 화면에서 어떻게 하는지 안내한다 — 우리가 대신 넣어 드리지 않는다.
...
ACTION_HINTS = {
    ...
    "CLOSE": "대화를 맺는다. 한 문장이면 된다 — 종료를 알리고 감사 인사를 한다. 새 정보를 덧붙이거나 다시 되묻지 않는다.",
}
...
def answer_prompt(
    ...
    # 종료 턴에는 근거를 넣지 않는다. 넣으면 "안내할 것이 있으면 다 안내하라"는
    # 규칙과 부딪쳐, 인사만 하면 될 자리에 점검 절차를 쏟아붓는다.
    if action != "CLOSE":
    ...
    if forbid_asks:
        items = ", ".join(str(a) for a in forbid_asks)
        parts.append(
            f"[다시 쓰기]\n앞서 쓴 답변이 고객에게 {items} 를 요구했다. 이번 턴에 물어야 할 것이 아니다 — "
            "고객이 아직 예약하지 않았을 수도 있고, 그것 없이도 안내할 수 있는 문의다. "
            f"{items} 를 묻지 말고, 근거 문서로 지금 안내할 수 있는 것을 안내해라. "
            "그래도 모자라면 [되물을 항목]에 적힌 것만 묻는다."
        )
    if forbid_numbers:
        # 검증에서 걸린 수치를 이름 붙여 돌려준다. "근거만 쓰라"고 다시 말하는 것보다
        # 무엇이 틀렸는지 짚어 주는 편이 고쳐질 확률이 높다.
        nums = ", ".join(str(n) for n in forbid_numbers)
        parts.append(
            f"[다시 쓰기]\n앞서 쓴 답변에 근거가 없는 수치가 있었다: {nums}\n"
            "이 수치는 조회 결과에도 근거 문서에도 없다. 쓰지 마라. 값을 모르면 "
            "금액을 말하지 말고 확인해서 안내하겠다고 해라. 나머지 안내는 그대로 해도 된다."
        )
