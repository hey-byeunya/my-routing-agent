...
- **CLOSE 는 고객이 아무것도 묻지 않았을 때만 고른다.** "없어요", "괜찮아요", "됐어요",
  "고맙습니다" 처럼 더 물을 것이 없다는 뜻의 짧은 응답이 그렇다. 이때는 이관이 아니다 —
  대화가 끝난 것이지 우리가 못 푼 것이 아니다.
  **물음이 하나라도 들어 있으면 CLOSE 가 아니다.** "처음이라 모르겠어요" 처럼 도움을
