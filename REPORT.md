...
| --- | --- | --- | --- | --- | --- |
| RESERVE_GENERAL | 12 | 4 | 3,119 | 1,902 | 1, 1.1, 1.2, 2 |
| VISIT_PICKUP | 13 | 5 | 3,454 | 2,122 | 1, 1.1, 2, 4, 4.1 |
| CVS_PICKUP | 14 | 6 | 3,485 | 2,144 | 1, 1.1, 2, 4, 4.3, 5.3 |
| BIZ_BULK | 17 | 9 | 3,545 | 2,243 | 1, 1.2, 4, 4.2, 4.4, 8.1, 8.2, 8.3, 9 |
| SPEC_SHIPPING | 14 | 6 | 2,876 | 1,810 | 3.1, 3.2, 5.1, 5.2, 6, 7 |
...
| 회차 | 변경 대상 | 핵심 변경 이유 (Why) | 핵심 변경 내용 (What) | 라우팅 (정확도 / macro F1) | 답변 (도구 / 적절성) | 성과 및 오답 메모 |
| --- | --- | --- | --- | --- | --- | --- |
...
답변 지표(도구 호출 적절성·답변 적절성)는 `openai` 로만 반복 측정했다. 한 번에 하나씩 바꾸고 매번 다시 재려면 회차마다 24턴을 돌려야 하는데, `opencode`·`ollama` 는 그 한 번이 각각 20분·42분이라 회차를 쌓을 수 없다. **그래서 백엔드 비교는 라우팅으로, 개선 회차는 openai 로** 갈랐다.
...
도구 호출은 24턴 전부 기대와 일치한다. 남은 것은 **답변 본문이 근거의 일부를 빠뜨리는** 문제다.
...
| `gate` (분기) | `route`, `confidence` | — (경로만 정함) | ✕ |
...
**② 채점기를 먼저 의심하는 순서.** `scripts/check_grader.py` 가 모범 답안 30턴을 채점해 전부 만점이 나오는지 본다. 여기서 두 번 걸렸다 — 서술형 `must_ask` 를 문자열로 못 잡던 것(채점기 결함)과, 다량할인에 택배사를 되물으라던 기준(정답셋 결함, 도구가 carrier 를 받지도 않는다). **이게 통과하기 전의 성능 수치는 믿지 않는다**는 순서를 지켰다.
...
- **답변 적절성 0.750 이 천장이 아니다.** 남은 6건은 근거의 일부를 빠뜨리는 문제로, 답변 프롬프트가 "무엇을 반드시 담아야 하는지"를 카테고리별로 더 구체적으로 줘야 한다.
- **멀티턴 평가의 카테고리 정답이 대화 단위다.** 첫 턴에서 서비스가 안 정해졌는데도 대화 전체의 카테고리를 정답으로 친다. 라우팅 정확도(36건 단일 턴)와 달리 이 수치는 그렇게 읽어야 한다.
