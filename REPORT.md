...
| #33<br>`openai/gpt-4o-mini` | `schemas.py RouteDecision · agent.py is_uncertain` | 모델이 스스로 매기는 확신도가 난이도를 못 알아챈다. 하드케이스에서 맞은 건 0.657 틀린 건 0.643 으로 사실상 같았고, 틀린 7건 중 τ 미만이 0건이라 게이트가 하나도 못 걸렀다 | 2순위 후보를 함께 받아 1·2순위 확신도 차(마진)가 좁으면 확신 안 섬으로 다룬다 | 0.881 / 0.879 (42건) | 1.000 / 0.935 (31턴) | 라우팅 0.800(대안 0.967) · 되물음 1.000 | 2순위 후보를 함께 요청한 뒤 평가셋 42건 재측정 (마진 게이트와 별개로 top-1 이 움직였는지 확인)<br>마진 게이트는 되돌리고 2순위 요청만 남긴 뒤 회귀 확인 |
...
경계가 갈리는 문의에 예시가 하나도 없었다. 그런 사례는 `hard_cases.csv` 에 있는데 **그게 곧 평가셋이라** 그대로 쓰면 누수가 된다. 그래서 먼저 dev/eval 을 갈랐다.
...
되물음 유형은 가르지 않았다 — 카테고리를 맞히는 문제가 아니고 이미 1.000 이라 지침을 벼릴 재료가 아니다. 분모가 라우팅 30 → 21 로 바뀌었으므로 **같은 21건에서 기준선을 다시 쟀다.**
...
**#35 규칙은 평평했다.** dev 에서 읽어낸 "공통 문의와 서비스 문의가 섞이면 서비스 쪽" 을 지침으로 적었는데 두 평가셋 모두 그대로였다. 근거가 없으므로 되돌렸다 — 재서 나아지지 않은 것을 프롬프트에 남기지 않는다.

둘 다 되돌렸지만 **dev/eval 분할은 남긴다.** 경계 사례로 프롬프트를 손대려면 언제든 필요하고, 그때 누수를 막는 것이 이 분할이다. `assert_hard_split_disjoint()` 가 평가를 돌릴 때마다 확인한다.
...
- **가드레일이 숫자만 본다.** 1,000 이상인 수를 근거에서 못 찾으면 잡는 방식이라, 금액 날조는 걸리지만 **말로 하는 단정은 못 걸린다.** "제주도 가능합니다", "취소 처리되었습니다" 같은 문장은 숫자가 없어 통과한다. 정작 매뉴얼 §10.3 이 금지한 것의 절반이 이쪽이다. #31 에서 숫자 쪽은 막게 했지만 말 쪽은 그대로다 — 다음은 여기다.
