...
| (없음) | `src/routing_agent/schemas.py`, `src/routing_agent/dataset.py` | 라우트 목록·구조화 출력 스키마와, 예시/평가 분리를 코드로 강제하는 적재기 |
...
| 셋 | 크기 | 무엇을 재나 |
| --- | --- | --- |
| `data/eval_set.csv` | 48건 (카테고리당 8건 균등) | 라우팅 분류 정확도·macro F1 |
...
**평가셋은 두 번 늘렸다.** 36 → 42건(#22, 경로 문의가 없었다) → 48건(#44, "서비스 + 규격 + 금액" 꼴이 없었다). 두 번 다 **빠진 갈래를 실제 결함으로 확인한 뒤** 늘렸다. 48건은 42건을 그대로 품은 상위집합이라 이전 회차와 같은 문항으로도 견줄 수 있다.
...
### 지금 수치 (gpt-4o-mini, 평가 31턴 / 라우팅 48건)

| | 기준선 (#4) | 지금 (#44) |
| --- | --- | --- |
| **도구 호출 적절성** | 0.750 | **1.000** |
| **답변 적절성** | 0.583 | **0.871** |
| 행동 일치 (진단용) | 0.625 | 0.742 |
| 라우팅 정확도 (단일 턴) | 0.889 (36건) | 0.854 (48건) |
| 폴백 | 0건 | 0건 |
| 가드레일 위반 | 1건 | 0건 |

라우팅은 LLM 없는 규칙 기준선 0.750 대비 0.854 다(둘 다 48건 기준). "LLM 을 썼으니 좋아졌다"가 아니라 **규칙으로도 얼마까지 되는지**를 먼저 재 두었다.

**42건에서 48건으로 늘리며 수치가 0.881 → 0.854 로 내려갔다. 성능이 나빠진 것이 아니다** — 새 평가셋은 옛 42건을 그대로 품은 상위집합이고, 그 42건의 성적은 0.881 그대로다. 더한 6건에서 4건을 맞혀(0.667) 전체가 내려갔다. **회차 간 라우팅 수치를 견줄 때는 #44 앞뒤로 분모가 다르다**(42건 → 48건).

행동 일치는 과제가 요구한 지표가 아니라 우리가 더 본 진단용 수치다. #18·#23 에서 내려갔는데, 뒤집힌 건들은 모두 **답변 내용은 통과하고 라벨만 흔들린** 것이다(ANSWER→ASK, ASK→CONFIRM, ANSWER→OUT_OF_SCOPE). 프롬프트 문구를 건드리면 이 라벨이 곧잘 흔들린다. 수치를 올리려고 되돌리지 않고 그대로 적는다. #41 에서 한 턴 더 내려갔다가(0.742 → 0.710) 그 회차를 되돌리면서 돌아왔다. 뒤집힌 턴(C-003#2)은 **기준선에서도 카테고리를 틀리던 턴**이고 답변 내용은 양쪽 다 통과했다.
...
| #45<br>`openai/gpt-4o-mini` | `prompts.py ROUTE_GUIDE_RULES` | — | 품목 문의 좁은 규칙(서비스 단서 없을 때만 SPEC_SHIPPING) | 0.833 / 0.827 (42건) | — | — | — |
| #46<br>`openai/gpt-4o-mini` | — | — | 같은 세션 기준선 재측정 | 0.881 / 0.879 (42건) | — | — | — |
| #47<br>`openai/gpt-4o-mini` | — | — | 품목 규칙 v2 — 서비스어가 있으면 규칙 적용 금지를 명시 | 0.857 / 0.853 (42건) | — | — | — |
| #48<br>`openai/gpt-4o-mini` | — | — | 품목 규칙 v2 회귀 확인 | — | — | 라우팅 0.800(대안 0.967) · 되물음 1.000 | — |
| #49<br>`openai/gpt-4o-mini` | — | — | 품목 규칙 v2 반복 측정 (변동폭 확인) | 0.881 / 0.879 (42건) | — | — | — |
| #41 | `prompts.py ROUTE_GUIDE_RULES` | 품목 이름만 있는 문의가 RESERVE_GENERAL 로 새어 반입 제한을 못 보던 것 | 서비스어가 없을 때만 SPEC_SHIPPING 으로 보내는 세 줄짜리 규칙 | openai/gpt-4o-mini 0.881 / 0.879 (42건) | openai/gpt-4o-mini 1.000 / 0.839 (31턴)<br>openai/gpt-4o-mini 1.000 / 0.871 (31턴)<br>openai/gpt-4o-mini 1.000 / 0.806 (31턴) | openai/gpt-4o-mini 라우팅 0.800(대안 1.000) · 되물음 1.000 | — |
| #50<br>`openai/gpt-4o-mini` | — | — | 같은 세션 기준선 (품목 규칙 없음) | — | 1.000 / 0.871 (31턴) | — | — |
| #51<br>`openai/gpt-4o-mini` | — | — | 품목 규칙 v2 반복 측정 | — | 1.000 / 0.839 (31턴) | — | — |
| #52<br>`openai/gpt-4o-mini` | — | — | 품목 규칙 v3 (3줄로 줄임) | 0.881 / 0.879 (42건) | — | — | — |
| #53<br>`openai/gpt-4o-mini` | — | — | 품목 규칙 v3 회귀 확인 | — | — | 라우팅 0.800(대안 1.000) · 되물음 1.000 | #41 앞에 회차 번호 없이 남은 측정들은 이 회차의 탐색 기록이다. routing_openai_base_s2·answer_openai_base_s2 는 같은 세션에서 다시 잰 기준선이고, item2(규칙 7줄)는 라우팅 0.833 으로 떨어져 버렸다. item3(서비스어가 있으면 규칙을 쓰지 말라고 명시)은 라우팅은 되돌아왔지만 답변 적절성이 0.871 → 0.839 로 1턴 깎였고, 캐시를 지우고 다시 재도 같은 값이 나와 변동폭이 아니라 실제 손실로 확인됐다. 채택한 것은 규칙을 세 줄로 줄인 item4(v3)다. |
| #54<br>`openai/gpt-4o-mini` | — | — | §7.2 수정 뒤 반복 측정 (변동폭) | — | 1.000 / 0.871 (31턴) | — | #41 마무리. §7.2 '포장 기준' 표 한가운데에 산문이 끼어 있어 표가 둘로 쪼개져 있던 것을 표 아래로 옮겼다. 직후 측정이 답변 0.871 → 0.806 으로 떨어졌지만 캐시를 비워 다시 재니 0.871 로 돌아왔다 — 변동폭이다. 바뀐 두 턴(C-001#2 운임 되물음, N-004#2 박스 규격)은 §7 과 무관한 문의였다. |
| #42 | `guardrail.py · agent.py verify · prompts.py` | 예약하지도 않은 고객에게 예약번호를 물어 대화가 막혔다 (화면, opencode) | 계획이 묻기로 한 것만 묻게 한다 — 답변이 지어낸 식별자 요구를 가드레일이 잡는다 | — | openai/gpt-4o-mini 1.000 / 0.839 (31턴)<br>openai/gpt-4o-mini 1.000 / 0.871 (31턴)<br>openai/gpt-4o-mini 1.000 / 0.871 (31턴) | — | — |
| #43 | `prompts.py ROUTE_GUIDE_RULES · mockdata · docs §4 · 정답셋` | #41 품목 규칙이 운임 문의를 SPEC_SHIPPING 으로 끌어가 데모 예시 ①이 답을 못 냈다. 운임표도 실제 값과 달랐다 | #41 규칙을 되돌리고, 2026-09-18 운임표로 목데이터·매뉴얼·정답셋을 맞췄다 | openai/gpt-4o-mini 0.881 / 0.879 (42건) | openai/gpt-4o-mini 1.000 / 0.871 (31턴)<br>openai/gpt-4o-mini 1.000 / 0.871 (31턴) | openai/gpt-4o-mini 라우팅 0.833(대안 0.967) · 되물음 1.000 | — |
| #44 | `data/eval_set.csv (42 → 48건)` | 평가셋이 '서비스+규격+금액' 꼴 문의를 한 건도 보지 않아 #41 의 결함을 못 잡았다 | 그 꼴을 카테고리마다 하나씩 더하고(대조군 둘 포함), 저작 문항이 균형 샘플링에서 빠지지 않게 고정한다 | openai/gpt-4o-mini 0.854 / 0.849 (48건)<br>openai/gpt-4o-mini 0.854 / 0.851 (48건)<br>rule 0.750 / 0.760 (48건) | — | rule 라우팅 0.767(대안 0.867) · 되물음 0.000 | — |

> 세대차: `round` 필드가 없던 시기의 레코드는 시간순으로 #0, #1 … 을 매겼다. 그 회차의 '변경 내용'은 실행 당시 `--note` 문구를 그대로 옮긴 것이고, '변경 이유'는 기록에 없어 비어 있다. 옛 줄은 고치지 않는다.

### 하드케이스 45건 (#29·#30)

평가셋(당시 42건)은 평범한 문의다. 분류기가 틀리기 쉬운 자리만 모은 `hard_cases.csv` 로 따로 쟀다.
...
| --- | ---: | --- |
| 기준선(#37) | 0.857 | — |
| 물건 이름 + "되나요" → 반입 제한(SPEC_SHIPPING) | **0.810** | 되돌림 (#41 에서 폭을 좁혀 다시 시도했다가 또 되돌렸다 — 아래) |
| OUT_OF_SCOPE 를 §10.1 네 유형으로 제한 | 0.857 | 평평 · 목표한 건도 못 고쳐 되돌림 |

첫 번째는 `수박`·`노트북` 을 §7 근거로 제대로 답하게 만들었지만 `"여러 박스도 방문택배로 되나요?"` 같은 문의까지 끌어당겼다. 경계를 넓히면 그만큼 엉뚱한 것이 딸려 온다. (이 건은 뒤에 #41 에서 규칙의 폭을 좁혀 다시 다뤘다 — 아래 "품목 문의 라우팅" 참조.)
...
### 품목 문의 라우팅 — 해 보고 되돌렸다 (#41)

`"딸기 보내려는데 되나요?"` 는 `RESERVE_GENERAL` 로 가서 §7 을 못 봤다. 백엔드를 갈아 끼워 보니 **모델마다 갈렸다.** 같은 지침으로 `claude/haiku` 와 `opencode` 는 `SPEC_SHIPPING` 0.85 로 맞히는데, `openai/gpt-4o-mini` 는 `RESERVE_GENERAL` 0.60 이었다. 판단 근거를 보면 이유가 드러난다 — `향수` 는 gpt-4o-mini 도 맞힌다(§7 에 `액체류` 가 적혀 있다). `딸기` 는 매뉴얼에 그 낱말이 없으니 "서비스가 특정되지 않은 문의"로 읽는다. **매뉴얼에 적힌 낱말만 반입 제한 문의로 알아본 것**이다. 적혀 있는지 확인하는 일이 바로 그 카테고리가 할 일인데 말이다.

#34 의 실패(라우팅 0.857 → 0.810)를 되풀이하지 않으려고 **규칙의 폭을 세 번 재며 좁혔다.**

| 규칙 문구 | 라우팅 42건 | 답변 적절성 31턴 | 판정 |
| --- | --- | --- | --- |
| #34 "물건 이름 + 되나요 → SPEC_SHIPPING" (7줄) | 0.810 | — | 되돌림 |
| v2 "서비스어가 없고 물건 이름만" + 예외 설명 (7줄) | 0.833 | — | `"도서지역인데 편의점택배로 보낼 수 있나요?"` 까지 끌어감 |
| v2' 위에 "서비스어가 있으면 이 규칙을 쓰지 마라" 명시 (9줄) | 0.881 | 0.839 | 라우팅은 회복했지만 답변이 1턴 깎임 |
| **v3 같은 내용을 세 줄로** | **0.881** | **0.871** | **채택** |

v2' 의 손실은 변동폭이 아니다. 캐시를 비우고 다시 재도 0.839 가 그대로 나왔다. 깎인 턴들(`"짐이 무거운데 편의점까지 들고 가기 힘들어요"`, `"개인도 쓸 수 있나요?"`)에는 **물건 이름이 하나도 없다** — 규칙이 겨냥한 문의가 아니라, 지침이 길어진 것 자체가 다른 판단을 흔든 것이다. 같은 내용을 세 줄로 줄이자 손실이 사라졌다. **무엇을 적는가만큼 얼마나 적는가가 성적을 바꾼다.**

**라우팅을 고치자 다음 결함이 드러났다.** `openai` 가 `SPEC_SHIPPING` 으로 가긴 했는데 답이 틀렸다 — *"딸기는 부패되기 쉬운 과일류로, 전용 박스 포장 시에만 접수 가능합니다"* (딸기는 어떻게 포장해도 접수 불가다). 근거 문서를 열어 보니 **§7.2 표 한가운데에 산문 한 문단이 끼어 있어 표가 둘로 쪼개져 있었다.** 사과·배의 예외를 설명하려고 넣은 문단인데, 그 자리에 있으니 연질 과일 줄 바로 뒤에 붙어 예외가 딸기에도 걸리는 것처럼 읽혔다. 문단을 표 아래로 옮기고 "이것들은 어떻게 포장해도 접수할 수 없습니다"를 명시하자 `openai` 도 *"딸기는 부패되기 쉬운 과일류에 해당하여 접수할 수 없습니다"* 로 바뀌었다. **`claude`·`opencode` 는 쪼개진 표로도 맞혔다** — 근거 문서의 결함을 강한 모델이 덮어 주고 있었던 것이고, 약한 모델로 돌려 본 덕에 드러났다.

v3 의 측정은 좋아 보였다: 라우팅 42건 0.881(기준선과 틀린 건까지 같다), 답변 31턴 1.000 / 0.871(기준선과 같음), 하드케이스 대안 인정 0.967 → 1.000. 품목 문의 5종(`딸기`·`사과`·`향수`·`자전거`·`노트북`)이 모두 `SPEC_SHIPPING` 으로 갔고, #34 를 무너뜨렸던 두 문의도 제 카테고리를 지켰다. **그래서 채택했는데, 틀렸다.**

**평가셋 밖에서 무너졌다.** 운임표를 갱신하고(#43) 데모 예시를 확인하다 드러났다 — `"롯데택배로 방문택배 보낼 건데 2kg에 60cm 이하예요. 얼마예요?"` 가 `SPEC_SHIPPING` 으로 갔다. 판단 근거에 규칙이 그대로 인용돼 있었다: *"특정 서비스에 대한 언급이 없으므로 반입 제한을 묻는 것으로 판단"*. 문장에 `방문택배` 도 `롯데택배` 도 있는데 말이다. `SPEC_SHIPPING` 근거에는 §4 운임표가 없고 운임 조회 도구도 붙지 않으므로, 답변은 *"정확한 운임을 확인해 드리겠습니다"* 라는 차단 문구로 끝났다. **REPORT 가 데모 증거로 싣고 있는 바로 그 문의다.**

같은 여덟 문의로 셋을 견줬다.

| 지침 | 품목 3건 | 운임 3건 | 경계 2건 | 합 |
| --- | --- | --- | --- | --- |
| 규칙 없음 | 1 | 1 | 2 | **4/8** |
| v3 (채택했던 것) | 3 | 0 | 2 | **5/8** |
| v3 + "값을 묻는 문의에는 쓰지 않는다" | 3 | 0 | 0 | **3/8** |

합계로는 v3 가 낫다. 그래도 되돌렸다. **잃는 쪽이 데모의 주 경로**이고(운임 문의는 도구를 부르지 못하면 답 자체가 없다), 얻는 쪽은 `gpt-4o-mini` 에서만 생기는 이득이기 때문이다 — `claude`·`opencode` 는 규칙 없이도 `딸기` 를 맞힌다. 예외를 덧붙여 좁히려 한 세 번째 시도는 경계 문의까지 무너뜨렸다.

**남은 것은 교훈 쪽이 크다.** 같은 착상을 세 번(#34·#41 v2·#41 v3) 시도해 세 번 되돌렸다. 그리고 **평가셋 42건이 이 결함을 한 번도 잡지 못했다** — "서비스 + 규격 + 금액" 꼴 문의가 평가셋에 없다. 수치가 평평하다는 것은 "나빠지지 않았다"가 아니라 "평가셋이 그 자리를 안 본다"일 수 있다. 이번에 그것을 실측으로 확인했다.

### 계획이 묻기로 한 것만 묻는다 (#42)

화면에서 나온 것이다. `"향수 보내려는데 되나요?"` → `"포장은 잘 되어있어요"` → `"한진택배"` 로 이어진 대화의 마지막 턴에 이렇게 답했다.

> 향수는 액체류로 접수가 제한될 수 있어 한진택배의 취급 기준 확인이 필요합니다. **예약번호를 알려주시면** 확인해서 안내드리겠습니다.

**아직 예약을 안 한 고객에게 예약번호를 요구했다.** 고객은 줄 수 없는 것을 요구받고, 우리는 §7 로 답할 수 있는 것을 안 답했다. 대화가 그 자리에서 막힌다. 판정은 `SPEC_SHIPPING` 0.92 로 맞았고 행동도 `ANSWER` 였으니 — **계획 단계는 아무것도 묻기로 하지 않았는데 답변 단계가 스스로 식별자를 지어내 물은 것**이다. `SPEC_SHIPPING` 근거에는 반입 제한(§7)과 배송조회(§5)가 함께 들어가는데, 답할 거리가 모자라자 옆 절의 식별자 요구를 끌어다 붙였다.

같은 결함을 #36 에서 프롬프트로 고쳐 본 적이 있다 — "고객이 없다고 한 것을 다시 묻지 않는다". 듣지 않아 뺐다. **그래서 또 검사로 옮겼다(§부탁 대신 검사로).** 기준은 애매하지 않다: **계획(plan)의 `ask` 에 없는 식별자를 답변이 요구하면 위반이다.** 계획이 `ASK` 로 예약번호를 묻기로 했으면 통과하고, 답변 단계가 혼자 지어낸 요구만 걸린다. 어느 쪽이 묻기로 했는지가 곧 기준이라 판단이 들어갈 자리가 없다.

**오탐 둘을 먼저 막아야 했다.** 평가셋을 돌리니 멀쩡한 턴 하나가 막혔다(C-007#2).

| 오탐 | 무엇이 문제였나 | 어떻게 막았나 |
| --- | --- | --- |
| `"수거 후 다시 확인해 주세요"` | `확인해 주세요` 는 **지시**이지 식별자 요구가 아니다 | 요청 표현에서 `확인해 주`·`주세요` 를 뺐다 |
| `"운송장번호 123456789인데 조회가 안 돼요"` 에 대한 답변 | 고객이 **이미 준** 번호는 다시 나오는 게 당연하다 | 고객 발화에 그 낱말이 있으면 흘린다 |

막은 뒤 측정: 답변 31턴 **도구 1.000 / 적절성 0.871 · 가드레일 위반 0 · 폴백 0** — 기준선과 같다. 검사 하나를 더 걸었는데 평가셋에서는 아무것도 걸리지 않았다는 뜻이고, 그게 맞다. 이 결함은 **평가셋에 없는 자리**에서 나왔다.

### 운임표를 실제 공개 값으로 갈았다 (#43)

방문택배·편의점택배 구간이 실제 공개 운임표와 달랐다. 사용자가 준 표로 목데이터를 갈았다.

| 어디 | 무엇이 바뀌었나 |
| --- | --- |
| 방문택배 | 롯데 네 구간 전부(3,800→4,000 / 4,200→4,400 / 4,950→5,000 / 5,950→6,000), CJ 첫 구간 크기 80→100cm 와 20kg 구간 5,990→6,000. **한진은 네 구간 모두 그대로였다** |
| 편의점택배 | CU·GS·이마트24 의 2kg·5kg 구간(3,490→3,500 / 3,750→3,800), 세븐일레븐 동일권·타권(3,070→3,400 / 3,470→3,900), 착한택배 1,800→1,980 |
| 새로 알게 된 사실 | **우체국택배는 제주·도서 할증이 없다.** 옛 데이터는 "확인 필요"로 비워 두고 있었다. 착한택배 제주 할증 3,000원도 새로 들어왔다 |

**정답셋 6턴의 기준도 함께 바꿨다.** 조회 결과가 달라졌으니 모범 답안이 말할 금액도 달라진다. 손으로 고치지 않고 `scripts/extend_goldenset.py` 의 `PATCHES` 에 적어 다시 돌려도 같은 결과가 나오게 했다(#39 의 C-010 과 같은 방식). rubric 에 **"수치를 올리려 기준을 느슨하게 한 것이 아니라 출처가 달라진 것"** 을 명시한다.

그중 하나는 **정답이 뒤집혔다.** N-001 은 "방문택배 2kg·60cm 에서 어디가 제일 싼가"를 묻는데, 옛 값으로는 롯데 3,800 < 한진 3,900 이었고 새 값으로는 **한진 3,900 < 롯데 4,000** 이다. 모범 답안의 택배사와 금액이 통째로 바뀌었다.

**픽스처도 두 개 깨졌고, 깨진 게 맞다.** `check_mockdb` 가 박아 둔 3,490·3,800 은 갱신된 값으로 고쳤다. `check_policy` 의 "근거 없는 금액" 예시는 6,000원을 쓰고 있었는데, 운임표 갱신으로 **6,000원이 §4.1 에 실제로 생기면서** 더 이상 근거 없는 값이 아니게 됐다 — 검사가 틀린 게 아니라 예시가 낡은 것이라, 아무 데도 없는 7,700원으로 바꿨다.

측정: 라우팅 42건 0.881 · 답변 31턴 **도구 1.000 / 적절성 0.871** · 하드케이스 엄격 0.833 · 대안 인정 0.967 · 되물음 1.000 · 가드레일 위반 0 · 폴백 0.

### 평가셋이 안 보던 자리를 메웠다 (#44)

#41 이 남긴 것은 규칙이 아니라 **평가셋의 빈자리**였다. 품목 규칙을 넣었을 때 평가셋 42건도 하드케이스 45건도 수치가 그대로였는데, 화면에서는 운임 문의가 통째로 무너져 있었다. 원본 90건의 운임 문의는 `"택배비가 얼마인가요?"` 처럼 **서비스도 규격도 없는** 꼴이고, 서비스·규격이 갖춰진 뒤 금액을 묻는 꼴 — **상담이 실제로 도구를 부르는 자리** — 이 한 건도 없었다.

카테고리마다 하나씩, 여섯 건을 저작해 더했다(`scripts/add_rate_inquiries.py`). 넷은 "서비스 + 규격 + 금액" 이고 둘은 대조군이다 — **금액이 나온다고 운임 카테고리로 끌어오지 않는지** 함께 본다.

| 더한 문의 | 정답 | 맞혔나 |
| --- | --- | --- |
| "2kg에 세 변 합 60cm인데 제일 싸게 보내려면 얼마예요?" | RESERVE_GENERAL | ✅ |
| "한진택배 방문택배로 5kg에 세 변 합 80cm면 얼마예요?" | VISIT_PICKUP | ✅ |
| "세븐일레븐 착한택배 5kg에 80cm 이하면 얼마예요?" | CVS_PICKUP | ❌ → RESERVE_GENERAL |
| "다량할인으로 10박스 보내는데 2kg에 60cm면 한 건당 얼마예요?" | BIZ_BULK | ❌ → RESERVE_GENERAL |
| (대조군) "운송장번호로 조회하면 결제한 금액도 같이 나오나요?" | SPEC_SHIPPING | ✅ |
| (대조군) "오픈마켓에서 결제한 배송비는 얼마나 환불되나요?" | OTHER | ✅ |

**틀린 둘은 둘 다 `RESERVE_GENERAL` 로 샌다.** 분류 지침의 *"서비스가 특정되지 않은 채 값만 묻는 문의는 RESERVE_GENERAL"* 을, 서비스(착한택배·다량할인)가 문장에 적혀 있는데도 적용한다. 지금 열려 있는 결함이고, **이제는 평가셋이 본다.**

**샘플러도 함께 고쳐야 했다.** 여섯 건을 더하고 평가셋을 다시 뽑았더니 **넷이 빠지고 둘만 남았다** — 등간격 샘플링이 qa_id 순으로 집으므로, 뒤에 붙인 문항은 그 카테고리에 원본이 많으면 그대로 떨어져 나간다. 메우려던 자리가 그대로 비어 있는 셈이다. 저작 문항을 먼저 집고 남은 자리를 등간격으로 채우게 했다. 다만 **꼭 필요한 것만 고정한다** — 경로 문의(#22)까지 고정했더니 정원에 밀려 원본 5건이 빠져 이전 회차와 같은 문항으로 견줄 수 없게 됐다. 운임 6건만 고정하니 **48건이 옛 42건을 그대로 품은 상위집합**이 된다.

**곁가지로 고친 것 — 규칙 기준선이 #33 이후 돌지 않고 있었다.** `--rule-baseline` 은 `RouteDecision` 을 직접 만드는데, #33 에서 `alt_route`·`alt_confidence` 를 필수 필드로 올리면서 그 자리가 깨졌다. 그때부터 지금까지 규칙 기준선을 한 번도 다시 재지 못했다는 뜻이다. 고쳐서 48건에서 다시 쟀다 — **규칙 0.750 · LLM 0.854**.

| | 42건 | 48건 |
| --- | --- | --- |
| 규칙 기준선 | 0.762 | 0.750 |
| openai/gpt-4o-mini | 0.881 | **0.854** (옛 42건만 보면 0.881 그대로, 더한 6건이 0.667) |
...
| C-001#2 [VISIT_PICKUP] | 되물음_누락 | 박스 크기와 중량 (서비스만 묻고 규격을 안 물었다) |
| C-011#2 [BIZ_BULK] | 되물음_누락 | 연동 시점 (쇼핑몰명·주문번호·발생일시는 물었다) |

도구 호출은 31턴 전부 기대와 일치한다. 남은 것은 **답변 본문이 근거의 일부를 빠뜨리거나, 되물을 것을 덜 묻는** 문제다.

앞 둘은 근거에 있는 사실을 빠뜨린 것이고, 뒤 둘은 **한 번에 물을 것을 나눠 물어 턴이 늘어나는** 쪽이다. 답 자체가 틀린 건은 없다.

이 기준들이 과한 것은 아닌지 정책 근거로 다시 확인했다 — §4.4(전월 발송량), §8.1(연동 이후 주문만 수집), §8.2(방문택배는 집하 상태), §0 원칙1(운임은 서비스·택배사·규격이 다 있어야 특정된다)로 전부 매뉴얼에 직접 근거가 있다. 정답셋을 느슨하게 고쳐 수치를 올리지 않았다.
...
| 문의 | 결과 |
| --- | --- |
| "롯데택배로 방문택배 보낼 건데 2kg에 60cm 이하예요. 얼마예요?" | `get_visit_rate` 호출 → 4,000원 인용, 가드레일 통과 |
...
**① 운임 조회가 필요한 문의** — `get_visit_rate` 를 부르고 그 결과(4,000원)로 답한다. 가드레일 통과.
...
**⑧ 수치가 평평한 것을 "안 나빠졌다"로 읽지 않기.** #41 에서 품목 규칙을 넣었을 때 평가셋 42건도 하드케이스 45건도 그대로였다. 그래서 채택했는데, 화면에서는 운임 문의가 통째로 무너져 있었다 — `SPEC_SHIPPING` 으로 새어 도구를 하나도 못 불렀다. **평가셋에 "서비스 + 규격 + 금액" 꼴이 한 건도 없어서 아무 일도 없는 것처럼 보였던 것이다.** 되돌리고(#43), 그 꼴을 평가셋에 넣었다(#44). 지금은 같은 변경을 하면 수치가 움직인다.

여기서 배운 순서는 이렇다. **평평한 수치를 만나면 먼저 "평가셋이 이 자리를 보는가"를 묻는다.** 그리고 손으로 열 건쯤 돌려 본다 — 평가셋이 안 보는 자리는 대개 거기서 먼저 드러난다. 이번에도 데모 예시를 확인하다 걸렸다.

**⑨ 되돌리기를 기록으로 남기기.** 같은 착상(품목 이름 → 반입 제한)을 세 번 시도해 세 번 되돌렸다(#34·#41 v2·#41 v3). 지운 것이 아니라 **왜 되돌렸는지를 수치와 함께** 남겼다. 세 번째에는 "합계로는 낫지만 잃는 쪽이 데모의 주 경로라 되돌린다"까지 적었다 — 수치만으로 정하지 않은 판단이라 근거를 남겨야 뒤에 다시 꺼낼 수 있다.
...
- **수치는 실행마다 흔들린다 — 라우팅 ±1건, 답변 ±2턴.** `temperature=0` 이어도 같은 코드로 캐시만 비우고 다시 재면 라우팅 42건이 0.881 → 0.857(#38), 답변 31턴이 0.871 → 0.806 → 0.871(#41) 로 움직인다. **회차 간 한두 건 차이는 변경의 효과가 아닐 수 있다.** #41 에서는 그래서 반대로 썼다 — 한 번 내려간 수치를 그대로 믿지 않고 캐시를 비워 다시 쟀고, 되풀이되면 실제 손실(v2'), 안 되풀이되면 변동폭(§7.2 수정)으로 갈랐다.
...
- **멀티턴 평가의 카테고리 정답이 대화 단위다.** 첫 턴에서 서비스가 안 정해졌는데도 대화 전체의 카테고리를 정답으로 친다. 라우팅 정확도(48건, 단일 턴)와 달리 이 수치는 그렇게 읽어야 한다.

**② 다음에 손댈 것**

- **서비스 이름이 있는데도 값을 물으면 `RESERVE_GENERAL` 로 샌다.** #44 에서 평가셋에 넣자마자 드러난 결함이다 — `"세븐일레븐 착한택배 5kg에 80cm 이하면 얼마예요?"`, `"다량할인으로 10박스 보내는데 …"` 가 둘 다 그렇다. 분류 지침의 *"서비스가 특정되지 않은 채 값만 묻는 문의는 RESERVE_GENERAL"* 을, 서비스가 문장에 적혀 있는데도 적용한다. 지침 문구로 밀어 본 시도는 세 번 다 되돌렸으니(#34·#41), 다음은 **규칙 라우터로 서비스어를 먼저 잡아 후보를 좁히는** 쪽이다.
...
- **가드레일이 숫자만 본다.** 1,000 이상인 수를 근거에서 못 찾으면 잡는 방식이라, 금액 날조는 걸리지만 **말로 하는 단정은 못 걸린다.** "제주도 가능합니다", "취소 처리되었습니다" 같은 문장은 숫자가 없어 통과한다. 정작 매뉴얼 §10.3 이 금지한 것의 절반이 이쪽이다. #31 에서 숫자 쪽은 막게 했지만 말 쪽은 그대로다. **#36 에서 실제로 터졌다** — "사과는 예약 취소 후에 가능합니다" 라는 없는 규정이 가드레일을 그냥 통과했다. #37 에서 기계적 검사를 붙였고(가능·불가를 단정하는 문장의 주어가 근거에 있는지), 그 뒤로 기간·기한(#40), ✗ 금지 예시 제외(#40), 계획에 없는 식별자 요구(#42), 맺지 않고 이어지는 단정("할 수 있지만", #42)까지 넓혔다. 다만 **글자만 맞춰 보는 검사라 낱말이 겹치면 못 가른다** — 정작 "사과"는 §10.2 의 "사과와 조치 권한" 때문에 근거에 있는 말로 잡혀 빠져나갔다(매뉴얼 쪽을 고쳐 막았다, #38). 뜻을 보는 검사가 다음 차례다.
- **답변 적절성 0.871 의 남은 4건.** 두 건(C-005#2 발송량 · N-005#2 집하)은 근거에 있는 조건을 답변에서 빠뜨린 것이고, 두 건(C-001#2 박스 크기·중량 · C-011#2 연동 시점)은 한 번에 물을 것을 나눠 물어 턴이 늘어나는 것이다. 프롬프트 문구를 고쳐 세 번 시도했고 두 번은 오히려 내려갔다(#15·#16). 문구로 밀어붙일 자리가 아니라, **카테고리별로 "이 답변에 반드시 들어가야 할 것"을 근거 문서에서 뽑아 넘기는** 구조가 필요해 보인다.
