...
- **`hard_cases.csv` 45건이 놀고 있다.** 경계모호 10 · 극단단답 8 · 다중의도 7 · 문맥의존 7 · 분류체계밖 6 · 텍스트손상 7 로, 분류기가 틀리기 쉬운 자리만 모은 셋이다. 병합 라벨(`route_expected_v2`)까지 붙여 뒀는데 측정에는 한 번도 쓰지 않았다. 지금 평가셋 42건은 평범한 문의라 **0.857 이 실제보다 후한 수치일 가능성이 크다.** 유형별로 나눠 재면 "어떤 종류의 문의에 약한가"가 처음으로 드러난다 — 남은 작업 중 값이 가장 크다.
