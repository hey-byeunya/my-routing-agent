...
행동 일치는 과제가 요구한 지표가 아니라 우리가 더 본 진단용 수치다. #18·#23 에서 내려갔는데, 뒤집힌 건들은 모두 **답변 내용은 통과하고 라벨만 흔들린** 것이다(ANSWER→ASK, ASK→CONFIRM, ANSWER→OUT_OF_SCOPE). 프롬프트 문구를 건드리면 이 라벨이 곧잘 흔들린다. 수치를 올리려고 되돌리지 않고 그대로 적는다. #41 에서 0.742 → 0.710 으로 한 턴 더 내려갔는데, 그 턴(C-003#2)은 **기준선에서도 카테고리를 틀리던 턴**이고 답변 내용은 양쪽 다 통과했다.
...
| #43<br>`openai/gpt-4o-mini` | `prompts.py ROUTE_GUIDE_RULES` | — | 품목 문의 좁은 규칙(서비스 단서 없을 때만 SPEC_SHIPPING) | 0.833 / 0.827 (42건) | — | — | — |
| #44<br>`openai/gpt-4o-mini` | — | — | 같은 세션 기준선 재측정 | 0.881 / 0.879 (42건) | — | — | — |
| #45<br>`openai/gpt-4o-mini` | — | — | 품목 규칙 v2 — 서비스어가 있으면 규칙 적용 금지를 명시 | 0.857 / 0.853 (42건) | — | — | — |
| #46<br>`openai/gpt-4o-mini` | — | — | 품목 규칙 v2 회귀 확인 | — | — | 라우팅 0.800(대안 0.967) · 되물음 1.000 | — |
| #47<br>`openai/gpt-4o-mini` | — | — | 품목 규칙 v2 반복 측정 (변동폭 확인) | 0.881 / 0.879 (42건) | — | — | — |
...
| #48<br>`openai/gpt-4o-mini` | — | — | 같은 세션 기준선 (품목 규칙 없음) | — | 1.000 / 0.871 (31턴) | — | — |
| #49<br>`openai/gpt-4o-mini` | — | — | 품목 규칙 v2 반복 측정 | — | 1.000 / 0.839 (31턴) | — | — |
| #50<br>`openai/gpt-4o-mini` | — | — | 품목 규칙 v3 (3줄로 줄임) | 0.881 / 0.879 (42건) | — | — | — |
| #51<br>`openai/gpt-4o-mini` | — | — | 품목 규칙 v3 회귀 확인 | — | — | 라우팅 0.800(대안 1.000) · 되물음 1.000 | #41 앞에 회차 번호 없이 남은 측정들은 이 회차의 탐색 기록이다. routing_openai_base_s2·answer_openai_base_s2 는 같은 세션에서 다시 잰 기준선이고, item2(규칙 7줄)는 라우팅 0.833 으로 떨어져 버렸다. item3(서비스어가 있으면 규칙을 쓰지 말라고 명시)은 라우팅은 되돌아왔지만 답변 적절성이 0.871 → 0.839 로 1턴 깎였고, 캐시를 지우고 다시 재도 같은 값이 나와 변동폭이 아니라 실제 손실로 확인됐다. 채택한 것은 규칙을 세 줄로 줄인 item4(v3)다. |
| #52<br>`openai/gpt-4o-mini` | — | — | §7.2 수정 뒤 반복 측정 (변동폭) | — | 1.000 / 0.871 (31턴) | — | #41 마무리. §7.2 '포장 기준' 표 한가운데에 산문이 끼어 있어 표가 둘로 쪼개져 있던 것을 표 아래로 옮겼다. 직후 측정이 답변 0.871 → 0.806 으로 떨어졌지만 캐시를 비워 다시 재니 0.871 로 돌아왔다 — 변동폭이다. 바뀐 두 턴(C-001#2 운임 되물음, N-004#2 박스 규격)은 §7 과 무관한 문의였다. |
| #42 | `guardrail.py · agent.py verify · prompts.py` | 예약하지도 않은 고객에게 예약번호를 물어 대화가 막혔다 (화면, opencode) | 계획이 묻기로 한 것만 묻게 한다 — 답변이 지어낸 식별자 요구를 가드레일이 잡는다 | — | openai/gpt-4o-mini 1.000 / 0.839 (31턴)<br>openai/gpt-4o-mini 1.000 / 0.871 (31턴) | — | — |
...
| --- | ---: | --- |
| 기준선(#37) | 0.857 | — |
| 물건 이름 + "되나요" → 반입 제한(SPEC_SHIPPING) | **0.810** | 되돌림 (#41 에서 폭을 좁혀 다시 성공) |
...
채택한 v3 의 측정: 라우팅 42건 0.881 (기준선과 **틀린 건까지 같다**), 답변 31턴 도구 1.000 / 적절성 0.871 (기준선과 같음), 하드케이스 45건 엄격 0.800 · 대안 인정 0.967 → **1.000** · 되물음 1.000. 품목 문의 5종(`딸기`·`사과`·`향수`·`자전거`·`노트북`)은 모두 `SPEC_SHIPPING` 으로 가고, #34 를 무너뜨렸던 `"여러 박스도 방문택배로 한 번에 되나요?"` 와 `"도서지역인데 편의점택배로 보낼 수 있나요?"` 는 각각 `VISIT_PICKUP`·`CVS_PICKUP` 을 지킨다.
