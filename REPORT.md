...
행동 일치는 과제가 요구한 지표가 아니라 우리가 더 본 진단용 수치다. #18 에서 0.833 → 0.750 으로 내려갔는데, 뒤집힌 2건은 **답변 내용은 통과하고 라벨만 흔들린** 것이다(ANSWER→ASK, ASK→CONFIRM). 수치를 올리려고 되돌리지 않고 그대로 적는다.
...
| 이전 대화 | 턴이 쌓여 다음 판정에 들어간다 — 반복 문의 이관(§10.2)이 화면에서도 성립한다 |
