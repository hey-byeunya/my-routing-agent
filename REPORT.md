...
| `data/answer_goldenset.json` | eval 20대화 / 채점 턴 29개 | 도구 호출 적절성·답변 적절성 |
...
### 지금 수치 (gpt-4o-mini, 평가 29턴 / 라우팅 42건)

| | 기준선 (#4) | 지금 (#18) |
| --- | --- | --- |
| **도구 호출 적절성** | 0.750 | **1.000** |
| **답변 적절성** | 0.583 | **0.897** |
| 행동 일치 (진단용) | 0.625 | 0.724 |
...
다만 **확신도가 낮다는 것과 사람이 필요하다는 것은 다르다.** 고객이 아직 정보를 덜 준 경우가 대부분이고, 그건 되물으면 풀린다. 그래서 게이트는 **같은 사안을 이미 되물었는데도** 확신이 서지 않을 때만 넘긴다(#25). 범위 밖(OTHER)은 확신도와 무관하게 넘기지 않는다 — 채널 안내로 끝낸다(#5).
