...
| #8<br>`openai/gpt-4o-mini` | `agent.py DEFAULT_THRESHOLD` | τ=0.6 은 측정치가 아니라 초기 휴리스틱이었고, 첫 문의 3건이 그 문턱에 걸려 이관됐다 | τ sweep(0.3/0.4/0.5/0.6) 후 0.4 로 확정. 0.3 과 동률이라 보수적인 쪽 | — | 1.000 / 0.667 (24턴) | τ=0.3<br>τ=0.4<br>τ=0.5<br>도구 0.875→1.000 · 행동 0.750→0.833. 평가셋 위에서 훑은 값이라 낙관적일 수 있음(REPORT 고지) |
...
| #13<br>`ollama/qwen2.5:3b` | `백엔드 비교` | 백엔드가 바뀌어도 같은 경로로 도는지, 수치가 얼마나 벌어지는지 | 같은 라우팅 평가셋 36건을 백엔드별로 | 0.000 / 0.000 (36건) | — | ollama qwen2.5:3b (로컬) |
